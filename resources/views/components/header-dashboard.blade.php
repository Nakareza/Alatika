    ...
    <div class="px-4 sm:px-6 lg:px-8 h-full">
        <div class="flex items-center justify-between h-full">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="flex items-center gap-3">
                        <h1 class="text-xl font-bold" style="color:#fff;font-family:'Plus Jakarta Sans',sans-serif;letter-spacing:-0.01em;">{{ $title }}</h1>
                        <div class="flex md:hidden items-center gap-2">
                            {{ $actions ?? '' }}
                        </div>
                    </div>
                    @if($subtitle)
                        <p class="text-xs mt-0.5" style="color:rgba(255,255,255,0.5);font-family:'Inter',sans-serif;">{{ $subtitle }}</p>
                    @endif
                    @if(count($breadcrumbs) > 0)
                        <nav class="flex items-center gap-1.5 text-xs mt-0.5" style="font-family:'Inter',sans-serif;">
                            <a href="{{ route('mahasiswa.dashboard') }}"
                               class="flex items-center gap-1.5 transition-colors"
                               style="color:rgba(255,255,255,0.5);"
                               onmouseover="this.style.color='#B5D4F4'"
                               onmouseout="this.style.color='rgba(255,255,255,0.5)'">
                                <img src="{{ asset('images/logo-polines.png') }}" alt="Logo" class="w-3.5 h-3.5 object-contain opacity-70">
                                Home
                            </a>
                            @foreach($breadcrumbs as $crumb)
                                <i class="fas fa-chevron-right text-xs" style="color:rgba(255,255,255,0.2);"></i>
                                @if(isset($crumb['url']))
                                    <a href="{{ $crumb['url'] }}"
                                       class="transition-colors"
                                       style="color:rgba(255,255,255,0.5);"
                                       onmouseover="this.style.color='#B5D4F4'"
                                       onmouseout="this.style.color='rgba(255,255,255,0.5)'">{{ $crumb['label'] }}</a>
                                @else
                                    <span style="color:rgba(255,255,255,0.85);font-weight:600;">{{ $crumb['label'] }}</span>
                                @endif
                            @endforeach
                        </nav>
                    @endif
            ...
            <div class="flex items-center gap-2">
                ...
                <div class="relative" x-data="{ open: false }">
                    ...
                    <div x-show="open"
                         ...
                         style="border:1px solid #EBF3FD;box-shadow:0 8px 32px rgba(30,43,74,0.20);display:none;">
                        <div class="px-4 py-2.5 border-b" style="border-color:#EBF3FD;">
                            <h4 class="font-bold text-sm" style="color:#1E2B4A;font-family:'Plus Jakarta Sans',sans-serif;">Notifikasi</h4>
                        </div>
                        <div class="max-h-72 overflow-y-auto">
                            <a href="#" class="flex gap-3 px-4 py-3 transition-colors border-b" style="border-color:#F5F8FF;"
                               onmouseover="this.style.background='#F5F8FF'" onmouseout="this.style.background=''">
                                <div class="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style="background:#EBF3FD;">
                                    <i class="fas fa-check text-xs" style="color:#185FA5;"></i>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs font-semibold truncate" style="color:#1E2B4A;font-family:'Plus Jakarta Sans',sans-serif;">Peminjaman Disetujui</p>
                                    <p class="text-xs mt-0.5" style="color:#94a3b8;font-family:'Inter',sans-serif;">Arduino Uno telah disetujui</p>
                                    <p class="text-xs mt-1" style="color:#B5D4F4;font-family:'Inter',sans-serif;">2 jam lalu</p>
                                </div>
                            </a>
                        </div>
                        <div class="px-4 py-2.5 border-t" style="border-color:#EBF3FD;">
                            <a href="#" class="text-xs font-semibold transition-colors"
                               style="color:#185FA5;font-family:'Plus Jakarta Sans',sans-serif;"
                               onmouseover="this.style.color='#1E2B4A'" onmouseout="this.style.color='#185FA5'">
                                Lihat Semua →
                            </a>
                        </div>
                    </div>
                ...
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open"
                        ...
                        <div class="hidden md:block text-left">
                            <p class="text-xs font-bold leading-tight" style="color:#fff;font-family:'Plus Jakarta Sans',sans-serif;">{{ Auth::user()->name }}</p>
                            <p class="text-xs" style="color:rgba(255,255,255,0.5);font-family:'Inter',sans-serif;">Mahasiswa</p>
                        </div>
                    ...
                    <div x-show="open"
                        ...
                        <button @click="open = false; $dispatch('open-modal-logout')"
                                ...
                                onmouseout="this.style.background=''">
                            <i class="fas fa-sign-out-alt w-4 text-center" style="font-size:12px;"></i>
                            Logout
                        </button>
                    </div>
                </div>
...
<x-modal name="logout" title="Konfirmasi Logout" type="danger">
    <p class="text-sm text-center mb-1" style="color:#94a3b8;font-family:'Inter',sans-serif;">
