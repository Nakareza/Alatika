...
<body class="bg-gray-50 antialiased" x-data="{ showLogoutModal: false, selectedRole: '{{ old('role', 'mahasiswa') }}' }">
    ...
    <div id="mainContent" class="transition-all duration-300 ease-in-out ml-64">
            ...
            <div class="px-4 sm:px-6 lg:px-8 py-4">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <a href="{{ route('kalab.users.index') }}" class="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">
                            <i class="fas fa-arrow-left"></i>
                        </a>
                        <div>
                            <h1 class="text-xl font-bold text-gray-900">Tambah User Baru</h1>
                            <p class="text-sm text-gray-500 mt-0.5">Buat akun untuk Admin, Mahasiswa, atau Kaprodi baru</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <main class="p-4 sm:p-6 lg:p-8 min-h-screen">
            <div class="max-w-2xl mx-auto">
                ...
                <form action="{{ route('kalab.users.store') }}" method="POST" class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    @csrf
                    ...
                    <div class="p-6 space-y-5">
                        ...
                        <div>
                            ...
                            <select name="program_studi" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                ...
                                <option value="D3 IK" {{ old('program_studi') === 'D3 IK' ? 'selected' : '' }}>D3 IK (D3 Teknik Informatika)</option>
                                <option value="D4 TRK" {{ old('program_studi') === 'D4 TRK' ? 'selected' : '' }}>D4 TRK (STr Teknologi Rekayasa Komputer)</option>
                            </select>
                        </div>
                    ...
                    <div class="px-6 py-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
                        <a href="{{ route('kalab.users.index') }}" class="text-sm text-slate-500 hover:text-slate-700 font-medium">
