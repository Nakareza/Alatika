        ...
        <header class="bg-white border-b border-gray-200 sticky top-0 z-30">
            <div class="px-8 py-4 flex justify-between items-center">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('mahasiswa.keranjang') }}" class="relative inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white font-medium rounded-xl hover:bg-indigo-700 transition shadow-sm">
                    ...
                    <a href="{{ route('mahasiswa.peminjaman.ajukan') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white font-medium rounded-xl hover:bg-blue-700 transition shadow-sm">
                        ...
                        @if($item->stok_tersedia > 0)
                            ...
                            <div x-show="open" x-transition class="fixed inset-0 bg-black/40 z-50 flex items-center justify-center" @click.self="open=false">
                                <div class="bg-white rounded-2xl shadow-2xl p-6 w-80 mx-4" @click.stop>
                                    <h3 class="font-bold text-gray-900 text-lg mb-1">{{ $item->nama }}</h3>
                                    <p class="text-sm text-gray-500 mb-5">Stok tersedia: <span class="font-semibold text-emerald-600">{{ $item->stok_tersedia }} unit</span></p>
                                    ...
                                    <div class="flex items-center gap-3 mb-6">
                                        ...
                                        <input type="number" x-model="jumlah" min="1" max="{{ $item->stok_tersedia }}"
                                            class="flex-1 text-center border border-gray-200 rounded-xl py-2 font-bold text-lg focus:ring-2 focus:ring-indigo-400 outline-none">
                                    ...
                                    <form action="{{ route('mahasiswa.keranjang.add', $item->id) }}" method="POST">
                                        @csrf
                                        ...
                                        <div class="flex gap-3">
                                            ...
                                            <button type="submit"
                                                class="flex-1 py-2.5 rounded-xl bg-indigo-600 text-white font-semibold text-sm hover:bg-indigo-700 transition shadow-sm">
