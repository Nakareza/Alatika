...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            {{-- Monitoring Barang Sedang Dipinjam --}}
            <div class="card p-6">
                <div class="flex items-center justify-between mb-5">
                    <div class="flex items-center gap-2">
                        <i class="fas fa-hand-holding text-[#185FA5]"></i>
                        <h2 class="text-lg font-bold" style="color:#1E2B4A;font-family:'Plus Jakarta Sans',sans-serif;">
                            Monitoring Peminjaman Aktif (Terbaru)
                        </h2>
                    </div>
                    <a href="{{ route('kaprodi.riwayat') }}" class="text-sm font-semibold transition-colors" style="color:#185FA5;" onmouseover="this.style.color='#1E2B4A'" onmouseout="this.style.color='#185FA5'">
                        Lihat Riwayat →
                    </a>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        <thead>
                            <tr class="bg-slate-50 border-b border-slate-100">
                                ...
                                <th class="px-6 py-3 text-xs font-semibold text-slate-500 uppercase">Alat</th>
                                <th class="px-6 py-3 text-xs font-semibold text-slate-500 uppercase">Jumlah</th>
                                <th class="px-6 py-3 text-xs font-semibold text-slate-500 uppercase">Tgl Kembali</th>
                                <th class="px-6 py-3 text-xs font-semibold text-slate-500 uppercase">Status</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            @forelse($active_borrowings as $peminjaman)
                            <tr class="hover:bg-slate-50 transition-all">
                                <td class="py-4 px-4">
                                    <div class="flex items-center gap-3">
                                ...
                                <td class="py-4 px-4">
                                    <div>
                                        <p class="text-sm text-slate-700 font-semibold {{ $peminjaman->isOverdue() ? 'text-red-600' : '' }}">
                                            {{ $peminjaman->tanggal_kembali->format('d M Y') }}
                                        </p>
                                        @if($peminjaman->isOverdue())
                                            <span class="text-xs font-semibold text-red-600 bg-red-50 px-2 py-0.5 rounded-full">Overdue {{ $peminjaman->days_overdue }} hari</span>
                                        @else
                                            <p class="text-xs text-slate-500">Dalam peminjaman</p>
                                        @endif
                                    </div>
                                </td>
                                <td class="py-4 px-4">
                                    <span class="badge {{ $peminjaman->status_config['color'] }}">
                                        <i class="fas {{ $peminjaman->status_config['icon'] }} mr-1"></i>
                            ...
                            <tr>
                                <td colspan="5" class="py-12 text-center text-slate-500">
                                    <i class="fas fa-hand-holding text-4xl mb-3 block text-slate-300"></i>
                                    <p class="font-medium">Tidak ada barang yang sedang dipinjam</p>
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
                ...
                <div class="space-y-2">
                    <a href="{{ route('kaprodi.persetujuan') }}"
                       class="flex items-center gap-3 p-3 rounded-xl hover:bg-blue-50 border border-transparent hover:border-blue-100 group transition-all">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-slate-400 font-medium">{{ $stats['pending'] }} menunggu</p>
                        </div>
                    </a>
                    <a href="{{ route('kaprodi.riwayat') }}"
                       class="flex items-center gap-3 p-3 rounded-xl hover:bg-blue-50 border border-transparent hover:border-blue-100 group transition-all">
                        ...
                        <div>
                            <p class="text-sm font-semibold text-slate-800">Riwayat & Monitoring</p>
                    ...
                    <a href="{{ route('kaprodi.profil') }}"
                       class="flex items-center gap-3 p-3 rounded-xl hover:bg-blue-50 border border-transparent hover:border-blue-100 group transition-all">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-slate-400 font-medium">Pengaturan Akun</p>
                        </div>
                    </a>
                </div>
            </div>
